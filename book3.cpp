...
using SeatOccupancy = unordered_map<string, int>;
...
void savePassengersToFile(const vector<Passenger> &passengers, const string &filename) {
    ...
    if (file.is_open()) {
        for (const Passenger &passenger : passengers) {
            file << passenger.name << " " << passenger.cruiseId << " " << passenger.accomodation << " " << passenger.waitListNumber << endl;
        }
        file.close();
    } else {
...
void schedulePassenger(vector<Cruise> &cruises, vector<Passenger> &passengers, const string &passengerName, const string &accommodation) {
    ...
    cout << YELLOW<<"Enter departure city: ";
    cin >> departureCity;
    ...
    cout << YELLOW<<"Enter the Cruise ID to schedule the passenger: "<<RESET;
    ...
    if (cruiseIt != matchingCruises.end()) {
        ...
        int selectedSeat;
        cout <<YELLOW<< "Enter the seat number you want to book: "<<RESET;
        cin >> selectedSeat;
        ...
        if (seatOccupied) {
            // If the selected seat is occupied, find the next available seat and add the passenger to the waiting list
            int waitListNumber = 0;
            for (const Passenger &passenger : passengers) {
                if (passenger.cruiseId == selectedCruiseId && passenger.accomodation == accommodation) {
                    waitListNumber++;
                }
            }
            Passenger newPassenger = {passengerName, selectedCruiseId, accommodation, waitListNumber};
            passengers.push_back(newPassenger);
            cout << "Seat " << selectedSeat << " is already occupied. Passenger added to waiting list with number " << waitListNumber << "." << endl;
...
void getPassengerStatus(const vector<Passenger> &passengers, const string &passengerName, const string &cruiseId)
{
    ...
    if (passengerIt != passengers.end())
    {
        ...
        cout << "Cruise ID: " << passenger.cruiseId << endl;
        cout << "Accomodation: " << passenger.accomodation << endl;

        if (passenger.waitListNumber != 0)
        {
            cout << "Waiting List Number: " << passenger.waitListNumber << endl;
        }
        else
        {
...
void addNewCruise(vector<Cruise> &cruises)
{
    ...
    cout << MAGENTA<<"Enter new cruise ID: ";
    cin >> id;
    ...
    cin >> regularSeats;
    cout << "Enter price for regular class: "<<RESET;
    cin >> regularPrice;
...
int main()
{
    ...
    do
    {
        ...
        case 's':
        {
            ...
            // cin >> cruiseId;
            cout <<MAGENTA<< "Enter passenger name: ";
            cin >> passengerName;
        ...
        case 'p':
        {
            ...
            cin >> cruiseId;
            cout << endl;
            cout << "The passenger details are as follows: " <<RESET<< endl;
        ...
        case 'N':
        {
            ...
            {
                cout << RED<<"Invalid admin password." <<RESET<< endl;
            }
            break;
        }
        case 'q':
            cout << MAGENTA<<"Exiting program.\n"<<RESET;
            break;
        default:
